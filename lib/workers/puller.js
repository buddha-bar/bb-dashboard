  ...
  function fetchOne(item) {
    async.waterfall([
      ...
      function(itemO, callback){
          ...
          console.log("Before etsy item change:", itemO.name);
          bEtsy.getItemCount(itemO.etsy.listingId, function(err, body){
            ...
            if (body.results[0].quantity < itemO.etsy.stock) {
              ...
              daEbay.getEbayItemCount(itemO.ebay.ItemID, function(err, body){
                if(err){
                  console.log(err);
                }
                var currentEbayStock = body;
    ...
    function(item, callback){
      ...
      function(err, result) {
        console.log(result.stock, results.etsy.stock, results.ebay.stock);
